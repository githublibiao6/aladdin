 ...
 **/
        ...
        try {
            ...
            //获取连接时最大等待时间，单位毫秒。当链接数已经达到了最大链接数的时候，应用如果还要获取链接就会出现等待的现象，等待链接释放并回到链接池，如果等待的时间过长就应该踢掉这个等待，不然应用很可能出现雪崩现："
            druidDataSource.setMaxWait(60000);
            //最小连接池数量
            druidDataSource.setMinIdle(40);
            String validationQuery = "select 1 from dual";

            // 申请连接时执行validationQuery检测连接是否有效，这里建议配置为TRUE，防止取到的连接不可："
            ...
            // 泄露连接的定义时："要超过最大事务的处理时间)；单位为秒。这里配置为1小时
...
//            setTargetDataSources(dynamicTargetDataSources2);// 将map赋值给父类的TargetDataSources
    ...
    */
    ...
    public void createDataSourceWithCheck(String key, DataSource dataSource) throws Exception {
        ...
        log.info("准备创建数据:"+datasourceId);
        log.info("数据:"+datasourceId+"之前已经创建，准备测试数据源是否正常...");
        ...
        try {
            ...
            log.info("数据:"+datasourceId+"正常");
        } catch (Exception e) {
            //把异常信息打印到日志文件
            log.error(e.getMessage(),e);
            rightFlag = false;
            log.info("缓存数据："+datasourceId+"已失效，准备删除...");
...
//        }
        if(testDatasource(driveClass,url,username,password)) {
            ...
            if(!result) {
                log.error("数据："+datasourceId+"配置正确，但是创建失：");
//                throw new ADIException("数据："+datasourceId+"配置正确，但是创建失：",500);
            }
        } else {
            log.error("数据源配置有错误");
