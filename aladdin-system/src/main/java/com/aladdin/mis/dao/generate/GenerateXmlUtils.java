...
public class GenerateXmlUtils {
    ...
    public static void writeXmlToFile(GeneratePo po, FileWriteFunction writeFn){
        ...
        String oldContent = writeFn.readContent(po.getFilePath(), po.getEntityName()+"Dao.xml");
        if(oldContent != null){
            oldContent = oldContent.substring(oldContent.indexOf("<!-- *************GenerateXmlUtils************** -->"));
            writeFn.write(content.toString() + "    " + oldContent,
            ...
            writeFn.write(content.toString(), po.getFilePath(), po.getEntityName()+"Dao.xml", true);
        }
    }
}
